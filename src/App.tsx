...
export type SearchTab = 'Title' | 'Message';

// --- UTILS & DATA PROCESSING ---
const CSV_URL = 'https://docs.google.com/spreadsheets/d/1FKCupmPRXSl8O0hVKHg8T1je0klmL7rBjOjbSPoeJws/export?format=csv&gid=2094350449';
      ...
      google.script.run
        .withSuccessHandler((response: string) => {
          try {
            ...
            const titleIdx = headers.findIndex(h => h.includes('id')) !== -1 ? headers.findIndex(h => h.includes('id')) : 0;
            const messageIdx = headers.findIndex(h => h.includes('message') || h.includes('mesage')) !== -1 ? headers.findIndex(h => h.includes('message') || h.includes('mesage')) : 1;
            const fileIdx = headers.findIndex(h => h.includes('filepathurl'));
            ...
            for (let i = 1; i < data.length; i++) {
              ...
              let message = String(row[messageIdx] || '').trim();
              if (fileIdx !== -1 && row[fileIdx]) {
                message += (message ? '\n\n' : '') + String(row[fileIdx]).trim();
              }
  ...
  Papa.parse(CSV_URL, {
    ...
    complete: (results) => {
      const templates: Template[] = results.data.map((row: any, index: number) => {
        ...
        const titleKey = keys.find(k => k.toLowerCase().includes('id')) || keys[0];
        const messageKey = keys.find(k => k.toLowerCase().includes('message') || k.toLowerCase().includes('mesage')) || keys[1];
        const fileKey = keys.find(k => k.toLowerCase().includes('filepathurl'));

        let message = row[messageKey]?.trim() || '';
        ...
        <div className="bg-white border-b border-slate-200 px-4 md:px-8 py-6 shrink-0 shadow-sm sticky top-[72px] z-20">
          <div className="space-y-4">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl w-full sm:w-fit overflow-x-auto gap-1">
              {(['Title', 'Message'] as SearchTab[]).map(tab => (
                <button
                  key={tab}
